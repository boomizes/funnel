...
### B. State Management & Persistence (`js/app.js`)
...
### C. PDF Generation Coordinates (`js/pdf-exporter.js`)
...
2.  **Responsive Layout**:
    ...
    *   Use the custom property `--vh` from `js/ui-utils.js` (managed dynamically in `js/app.js` on window resize) rather than standard `100vh` to prevent jumping scrollbars and viewport clipping in Mobile Safari/Chrome.
